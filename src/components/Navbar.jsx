...
import { ShoppingCart, Search, Menu, Phone, MessageCircle, Star } from 'lucide-react';
...
const Navbar = () => {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300">
            <nav className={`transition-all duration-300 ${isScrolled ? 'bg-white shadow-xl py-3' : 'bg-white/90 backdrop-blur-md py-5'}`}>
                <div className="container-custom flex justify-between items-center">
                    {/* Logo */}
                    <Link to="/" className="flex items-center group">
                        <img src="/logo.png" alt="M J Optics" className="h-10 md:h-12 w-auto object-contain transition-transform group-hover:scale-105" />
                    </Link>
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <a
                            ...
                            className="hidden md:flex items-center gap-2 bg-[#25D366] text-white px-5 py-2.5 rounded-full text-xs font-black hover:bg-opacity-90 transition-all shadow-md active:scale-95"
                        >
